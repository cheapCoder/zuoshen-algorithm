...
public class C02_CoinsMin {
	...
	private static int process(int[] arr, int rest, int count) {
		if (rest == 0) {
			return count;
		}
		if (rest < 0 || count > rest) {
			return -1;
		}
		int tem = Integer.MAX_VALUE;
		for (int i = 0; i < arr.length; i++) {
			int next = process(arr, rest - arr[i], count + 1);
			if (next != -1) {
				tem = Math.min(tem, next);
			}
		}
		return tem != Integer.MAX_VALUE ? tem : -1;
	}

	//NOTE: count范围不确定，未想到怎么用严格表结构优化; 用上面注释掉的minCoins1可优化出严格表结构

	// for test
	public static int[] generateRandomArray(int len, int max) {
		int[] arr = new int[(int) (Math.random() * len) + 1];
		for (int i = 0; i < arr.length;) {
			int tem = (int) (Math.random() * max) + 1;
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < testTime; i++) {
			...
			int aim = (int) (Math.random() * 2 * max) + max;
